...
local default_settings = {
	...
	commands_manage = {
		...
		{ cmd = 'inv' , description = 'Принятие игрока в фракцию' , text = '/do В кармане есть связка с ключами от раздевалки.&/me достаёт из кармана один ключ из связки ключей от раздевалки&/todo Возьмите, это ключ от нашей раздевалки*передавая ключ человеку напротив&/invite {arg_id}' , arg = '{arg_id}', enable = true, waiting = '1.200' , deleted = false  },
...
local InfoWindow = imgui.new.bool()
...
function calculateZoneRu(x, y, z)
    local streets = {
        ...
        {"Грузовое депо ЛВ", 1236.630, 1163.410, -89.084, 1277.050, 1203.280, 110.916},
        ...
        {"Грузовое депо ЛВ", 1277.050, 1087.630, -89.084, 1375.600, 1203.280, 110.916},
        ...
        {"Грузовое депо ЛВ", 1375.600, 919.447, -89.084, 1457.370, 1203.280, 110.916},
        ...
        {"Грузовое депо ЛВ", 1457.390, 863.229, -89.084, 1777.400, 1143.210, 110.916},
			...
			if imgui.BeginTabItem(fa.HOUSE..u8' Главное меню') then
				...
				if imgui.BeginChild('##3', imgui.ImVec2(589 * MONET_DPI_SCALE, 127 * MONET_DPI_SCALE), true) then
					...
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"Автоматически надевает маску когда она слетает")
					end
					imgui.SetColumnWidth(-1, 230 * MONET_DPI_SCALE)
					imgui.NextColumn()
					if settings.general.auto_mask then
					...
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"Сообщения будут отправлятся с заглавной буквы и с точкой в конце\nРаботает в обычном чате и некоторых популярных командах")
					end
					imgui.NextColumn()
					if settings.general.rp_chat then
						imgui.CenterColumnText(u8'Включено')
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 9, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		...
		imgui.Separator()
		imgui.Text(fa.CLOCK .. u8(' Время: ') .. u8(tagReplacements.get_time()))
		imgui.End()
    end
)
