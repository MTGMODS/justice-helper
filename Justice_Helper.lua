...
local default_settings = {
	...
	commands = {
		...
		{ cmd = 'zd' , description = 'Привествие игрока' , text = 'Здраствуйте, я {my_ru_nick} - {fraction_rank} {fraction_tag}&Чем я могу Вам помочь?', arg = '{arg_id}' , enable = true , waiting = '1.200', deleted = false },
        { cmd = 'cure' , description = 'Поднять игрока из стадии' ,  text = '/me наклоняется над человеком, затем прощупывает пульс на сонной артерии&/cure {arg_id}&/do Пульс отсутствует.&/me начинает делать человеку непрямой массаж сердца, время от времени проверяя пульс&/do Спустя несколько минут сердце человека началось биться.&/do Человек пришел в сознание.&/todo Отлично*улыбаясь' , arg = '{arg_id}' , enable = true , waiting = '1.200', deleted = false  },
		{ cmd = 'time' , description = 'Посмотреть время' ,  text = '/me взглянул{sex} на свои часики и посмотрел{sex} время&/time&/do На часах видно время {get_time}.' , arg = '' , enable = true, waiting = '1.200' , deleted = false },
        { cmd = 'pas' , description = 'Запросить документы (PD)' ,  text = 'Здраствуйте, управление {fraction_tag}, я {fraction_rank} {my_ru_nick}&/do Cлева на груди жетон полицейского, справа именная нашивка с именем.&/me  достаёт своё удостоверение из кармана&/showbadge {arg_id}&Прошу предъявить документ, удостоверяющий вашу личность.&/n {get_nick({arg_id})}, введите /showpass {my_id}' , arg = '{arg_id}' , enable = true , waiting = '1.200', deleted = false  },
        { cmd = 'pass' , description = 'Запросить документы (FBI)' ,  text = 'Здраствуйте, я {fraction_rank} ФБР&/do Cлева на груди спец-жетон ФБР.&Прошу предъявить документ, удостоверяющий вашу личность.&/n {get_nick({arg_id})}, введите /showpass {my_id}' , arg = '{arg_id}' , enable = false , waiting = '1.200', deleted = false  },
        { cmd = 'tt' , description = 'Выписать штраф' ,  text = '/do Бланк протокола и ручка находяться в нагрудном кармане.&/me достаёт из нагрудного кармана бланк протокола и ручку.&/me вписывает в бланк данные нарушителя&/do Бланк протокола заполнен.&/me передаёт бланк со штрафом нарушителю для дальнейшей оплаты&/writeticket {arg_id} {arg2} {arg3}' , arg = '{arg_id} {arg2} {arg3}' , enable = true, waiting = '1.200' , deleted = false },
        { cmd = 'pr' , description = 'Погоня' ,  text = '/pursuit {arg_id}' , arg = '{arg_id}' , enable = true, waiting = '1.200' , deleted = false },
        { cmd = 'su' , description = 'Выдать розыск' ,  text = '/me достаёт свой КПК и открывает базу данных преступников&/me вносит изменения в базу данных преступинков&/do Преступник занесён в базу данных преступиков.&/su {arg_id} {arg2} {arg3}&/z {arg_id}' , arg = '{arg_id} {arg2} {arg3}' , enable = true, waiting = '1.200' , deleted = false },
        { cmd = 'fsu' , description = 'Запросить выдачу розыска' ,  text = '/do Рация на тактическом поясе.&/me достаёт рацию и связавается с диспетчером&/me передаёт диспетчеру запрос на внесение человека в базу данных преступников&{my_doklad_nick} на CONTROL&Прошу обьявить в розыск {arg2} степени дело N-({arg_id}). Причина: {arg3}' , arg = '{arg_id} {arg2} {arg3}' , enable = true, waiting = '1.200' , deleted = false },
		{ cmd = 'unsu' , description = 'Понизить розыск' ,  text = '/me достаёт свой КПК и открывает базу данных преступников&/me ищёт дело N-({arg_id}) и вносит изменения в базу данных преступинков&/unsu {arg_id} {arg2} {arg3}&/do Преступику понижен степень розыска.' , arg = '{arg_id} {arg2} {arg3}' , enable = true, waiting = '1.200' , deleted = false },
		{ cmd = 'clear' , description = 'Снять розыск' ,  text = '/me достаёт свой КПК и открывает базу данных преступников&/me ищёт дело N-({arg_id}) и вносит изменения в базу данных преступинков&&/clear {arg_id}/do Дело N-({arg_id}) больше не находится в списке разыскиваемых преступников.' , arg = '{arg_id}' , enable = true, waiting = '1.200' , deleted = false },
        { cmd = 'cuff' , description = 'Надеть наручники' ,  text = '/do Наручники на тактическом поясе.&/me снимает наручники с пояса и надевает их на задержанного&/cuff {arg_id}&/do Задержанный в наручниках.' , arg = '{arg_id}' , enable = true , waiting = '1.200', deleted = false},
        { cmd = 'uncuff' , description = 'Снять наручники' ,  text = '/do На тактическом поясе прикреплены ключи от наручников.&/me снимает с пояса ключ от наручников и вставляет их в наручники задержанного&/me прокручивает ключ в наручниках и снимает их с задержанного&&/uncuff {arg_id}&/do Наручники сняты с задержанного&/me кладёт ключ и наручники обратно на тактический пояс', arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false},
        { cmd = 'gtm' , description = 'Повести за собой' ,  text = '/me схватывает задержанного за руки и ведёт его за собой&/gotome {arg_id}&/do Задержанный идёт в конвое.', arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false},
        { cmd = 'ungtm' , description = 'Перестать вести за собой' ,  text = '/me отпускает руки задержанного и перестаёт вести его за собой&/ungotome {arg_id}', arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false},
		{ cmd = 'ss' , description = 'Кричалка' ,  text = '/s Всем поднять руки вверх, работает {fraction_tag}!', arg = '', enable = true, waiting = '1.200', deleted = false},
		{ cmd = 't' , description = 'Достать тазер' ,  text = '/taser', arg = '', enable = true, waiting = '1.200', deleted = false},
		{ cmd = 'frl' , description = 'Первичный обыск' ,  text = 'Сейчас я проверю у вас наличие оружия или других острых предметов, не двигайтесь.&/me прощупывает тело задержанного человека&/me прощупывает карманы задержанного человека', arg = '', enable = false, waiting = '1.200', deleted = false},
		{ cmd = 'fr' , description = 'Полный обыск' ,  text = '/do Резиновые перчатки на тактическом поясе.&/todo Сейчас я полностю обыщу вас, на наличие запрещенных предметов*надевая резиновые перчатки&/me прощупывает тело и карманы задержанного человека&/me достаёт из карманов задержанного все его вещи для изучения&/me внимательно осматривает все найденные вещи у задержанного человека&/frisk {arg_id}&/me снимает резиновые перчатки и убирает их на тактический пояск&/do Блокнот с ручкой в нагрудном кармане.&/me берет в руки блокнот с ручкой, и записывает всю информацию про обыск&/me сделал пометки, убирает блокнот с ручкой в нагрудный карман', arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false},
		{ cmd = 'inc' , description = 'Затащить в крузер' ,  text = '/me открывает заднюю дверь крузера&/todo Наклоните голову, здесь дверь*затаскивая задержанного в транспортное средство&/incar {arg_id} {arg2}&/me закрывает заднюю дверь крузера&/do Задержанный в транспортном средстве.', arg = '{arg_id} {arg2}', enable = true, waiting = '1.200', deleted = false},
		{ cmd = 'ej' , description = 'Выбросить из крузера',  text = '/me открывает дверь крузера&/me помогает человеку выйти из крузера&/eject {arg_id}&/me закрывает дверь крузера', arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false},	
		{ cmd = 'pl' , description = 'Выбросить игрока из его т/с',  text = '/me резким ударом дубинки разбивает стело крузера и вскрывает дверь&/pull {arg_id}&/me выбрасывает задержанного из т/с и ударом дубинки оглушает его', arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false},	
		{ cmd = 'mr' , description = 'Зачитать правило Миранды',  text = 'Вы имеете право хранить молчание.&Всё, что вы скажете, может и будет использовано против вас в суде.&Вы имеете право на 1 телефонный звонок, например для вызова частного адвоката.&Ваш адвокат может присутствовать при допросе.&Если вы не можете оплатить услуги адвоката, он будет предоставлен вам государством.&Вам ясны Ваши права?', arg = '', enable = true, waiting = '1.200', deleted = false},	
		{ cmd = 'unmask' , description = 'Снять балаклаву с игрока',  text = '/do Задержанный в балаклаве.&/me стягивает балаклаву с головы задеражнного&/unmask {arg_id}', arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false},
		{ cmd = 'arr' , description = 'Арестовать (в участке)',  text = '/me включает свой бортовой компютер и вводит код доступа сотрудника&/me заходит в раздел оформления протоколов задержаний и указывает данные&/do Протокол задержания заполнен.&/me вызывает по рации дежурный наряд участка и передаёт им задержанного человека&/arrest {arg_id}', arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false},
		{ cmd = 'mask' , description = 'Надеть балаклаву' ,  text = '', arg = '', enable = true, waiting = '1.200', deleted = false},
		{ cmd = 'smask' , description = 'Снять балаклаву' ,  text = '', arg = '', enable = true, waiting = '1.200', deleted = false},
		{ cmd = 'drugs' , description = 'Провести Drugs Test' ,  text = '/do На тактическом поясе прикреплён подсумок.&/me открывает подсумок и достаёт из него набор Drugs Test&/me берёт из набора пробирку с этиловым спиром&/me засыпает найденное вещество в пробирку&/me достаёт из подсумка тест Имуно-Хром-10 и добавляет его в пробирку&/do В пробирке с этиловым спиртом находится неизвестное вещество и Имуно-Хром-10.&/me акуратными движениями взбалтывает пробирку&/do От теста Имуно-Хром-10 содержимое пробирки сменило цвет.&/todo Да, это точно наркотики*увидев что содержимое пробирки сменило цвет&/me убирает пробирку обратно в подсумок и закрывает его', arg = '', enable = true, waiting = '1.200', deleted = false},
		{ cmd = 'rbomb' , description = 'Деактивировать бомбу' ,  text = '/do На тактическом поясе прикреплён сапёрный набор.&/me снимает с пояса сапёрный набор и кладет его на землю, затем открывает его&/do Открытый сапёрный набор находится на земле.&/me достаёт из сапёрного набора пакет с жидким азотом и кладет его на землю&/me достаёт из сапёрного набора отвёртку&/do Отвертка в руках, а пакет с жидким азотом на земле.&/do На корпусе бомбы находится 2 болтика.&/me откручивает болтики с бомбы и убирает их вместе с отвёрткой в сторону&/me акуратным движением руки вскрывает крышку бомбы&/me внимательно осматривает бомбу&/do Внутри бомбы видна детонирующая часть.&/me достаёт из сапёрного набора кусачки&/do Кусачки в руках.&/me акуратным движением кусочок разрезает красный провод бомбы&/do Таймер остановился, тиканье со стороны бомбы не слышно.&/me берёт в руки охлаждающий пакет с жидким азотом и кладёт его детонирующую часть бомбы&/removebomb&/do Бомба обезврежена.&/me убирает кусачки и отвёртку обратно в саперный набор и закрывает его', arg = '', enable = true, waiting = '1.200', deleted = false},
	...
	commands_manage = {
		...
		{ cmd = 'inv' , description = 'Принятие игрока в фракцию' , text = '/do В кармане халата есть связка с ключами от раздевалки.&/me достаёт из халата один ключ из связки ключей от раздевалки&/todo Возьмите, это ключ от нашей раздевалки*передавая ключ человеку напротив&/invite {arg_id}' , arg = '{arg_id}', enable = true, waiting = '1.200' , deleted = false  },
		...
		if not isActiveCommand then
			if isParamSampID(arg) then
				if #smart_uk == 0 then
					...
					sampAddChatMessage('[Justice Helper] {ffffff}Сначало загрузите/отредакртируйте умный розыск в /hh', message_color)
				end
			else
		...
		sampRegisterChatCommand("spcar", function()
			if not isActiveCommand then
				lua_thread.create(function()
					...
					sampSendChat("/rb Внимание! Через 15 секунд будет спавн т/с организации.")
					wait(1500)
					if command_stop then 
						command_stop = false 
						isActiveCommand = false
...
local rpTake = {
	[2]=1, [5]=1, [6]=1, [7]=1, [8]=1, [9]=1, [14]=1, [15]=1, [25]=1, [26]=1, [27]=1, [28]=1, [29]=1, [30]=1, [31]=1, [32]=1, [33]=1, [34]=1, [35]=1, [36]=1, [37]=1, [38]=1, [42]=1, [77]=1, [78]=1, [78]=1, [79]=1, [80]=1, [81]=1, [82]=1, [83]=1, [84]=1, [85]=1, [86]=1, [92]=1, [87]=1, [88]=1, -- спина
	[1]=2, [4]=2, [10]=2, [11]=2, [12]=2, [13]=2, [41]=2, [43]=2, [44]=2, [45]=2, [46]=2, -- карман
	[16]=3, [17]=3, [18]=3, [39]=3, [40]=3, [90]=3, [91]=3, -- пояс
	[3]=4, [22]=4, [23]=4, [24]=4, [71]=4, [72]=4, [73]=4, [74]=4, [75]=4, [76]=4, [89]=4, -- кобура
}
			...
			if type == "finished" then
				...
				if updateInfo then
					...
					if thisScript().version ~= uVer then
						print('[Justice Helper] Доступно обновление! Используйте /update')
						updateUrl = uUrl
						updateVer = uVer
...
function sampev.onSendTakeDamage(playerId,damage,weapon)
	playerId2 = playerId1
		...
		if isParamSampID(playerId) and playerId1 ~= playerId2 then
			sampAddChatMessage('[Justice Helper] {ffffff}Игрок ' .. sampGetPlayerNickname(playerId) .. '[' .. playerId .. '] напал на вас используя ' .. weapons.get_name(weapon) .. '!', message_color)
			table.insert(enemy, 'Игрок ' .. sampGetPlayerNickname(playerId) .. '[' .. playerId .. '] напал на вас используя ' .. weapons.get_name(weapon) .. '!')
		end
end
function sampev.onServerMessage(color,text)
	...
	if text:find("Вы успешно надели маску") or text:find("Теперь вы в маске") then
		maska = true
...
function sampev.onShowDialog(dialogid, style, title, button1, button2, text)
	...
	if title:find('Основная статистика') and check_stats then -- получение статистики
		...
		if text:find("{FFFFFF}Организация: {B83434}%[(.-)]") then
			...
			if settings.player_info.fraction == 'Не имеется' then
				...
				elseif settings.player_info.fraction == 'FBI' then
	...
	-- 		local ammo = text:match('{FFFFFF}Патроны: 	%[(%d+) ед%]')
	-- 		if ammo > 0 then
	-- 			text = text .. ' ' .. ammo .. 'патронов' 
	-- 		end
	-- 	end
	-- 	if text:find('{FFFFFF}Семена наркотиков: 	%[(%d+) ед%]') then
	-- 		local narko = text:match('{FFFFFF}Семена наркотиков: 	%[(%d+) ед%]')
	-- 		if narko > 0 then
	-- 			text = text .. ' ' .. narko .. 'семян' 
	-- 		end
	-- 	end
	-- 	if text:find('{FFFFFF}Наркотики: 	%[(%d+) ед%]') then
	-- 		local narko = text:match('{FFFFFF}Наркотики: 	%[(%d+) ед%]')
	-- 		if narko > 0 then
	-- 			text = text .. ' ' .. narko .. 'грам вещества' 
	-- 		end
	-- 	end
	-- 	if text:find('{FFFFFF}Адреналин: 	%[(%d+) ед%]') then
	-- 		local adr = text:match('{FFFFFF}НАдреналин: 	%[(%d+) ед%]')
	-- 		if adr > 0 then
	-- 			text = text .. ' ' .. adr .. 'таблеток адреналина' 
	-- 		end
	-- 	end
	-- 	if text:find('{FFFFFF}Отмычки от тюремных дверей: 	%[(%d+) ед%]') then
	-- 		local tsr = text:match('{FFFFFF}Отмычки от тюремных дверей: 	%[(%d+) ед%]')
	-- 		if tsr > 0 then
	-- 			text = text .. ' ' .. tsr .. 'отмычек' 
	-- 		end
			
	-- 	end
	-- 	if not text:find('{FFFFFF}Оружие: 	{FF0000}%[Нет%]') then
	-- 		text = text .. ' оружие' 
	-- 	end
	-- end
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.HANDCUFFS.." Justice Helper##main", MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize )
		if imgui.BeginTabBar('Tabs') then	
			if imgui.BeginTabItem(fa.HOUSE..u8' Главное меню') then
				...
				if imgui.BeginChild('##2', imgui.ImVec2(589 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE), true) then
					imgui.CenterText(fa.CIRCLE_INFO .. u8' Текст')
					imgui.Separator()
					imgui.Columns(3)
				...
				if imgui.BeginChild('##3', imgui.ImVec2(589 * MONET_DPI_SCALE, 125 * MONET_DPI_SCALE), true) then
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText(u8"Автоматическая маска")
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"Авто надевание маски когда она слетает")
					end
					imgui.SetColumnWidth(-1, 230 * MONET_DPI_SCALE)
					imgui.NextColumn()
					if settings.general.auto_mask then
					...
					imgui.NextColumn()
					if settings.general.auto_mask then
						if imgui.CenterColumnSmallButton(u8'Отключить') then
							settings.general.auto_mask = false
							save_settings()
						end
                    else
						if imgui.CenterColumnSmallButton(u8'Включить') then
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText(u8"RP Оружие")
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"RP отыгровки для оружия")
					end
					imgui.NextColumn()
					...
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip(u8"Сообщения в чат автоматически будут отправлятся с заглавной буквы и с точкой в конце")
					end
					imgui.NextColumn()
					if settings.general.rp_chat then
						imgui.CenterColumnText(u8'Включено')
			...
			if imgui.BeginTabItem(fa.STAR .. u8' Розыск и штрафы') then 
				imgui.BeginChild('##smartuk', imgui.ImVec2(292 * MONET_DPI_SCALE, 360 * MONET_DPI_SCALE), true)
						...
						if imgui.BeginPopupModal(u8(chapter.name) .. '##' .. chapter_index, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
							if imgui.BeginChild('##smartukedititem', imgui.ImVec2(589 * MONET_DPI_SCALE, 390 * MONET_DPI_SCALE), true) then
								if chapter.item then
									for index, item in ipairs(chapter.item) do
										...
										imgui.SetColumnWidth(-1, 520 * MONET_DPI_SCALE)
										imgui.NextColumn()
							...
							if imgui.BeginPopupModal(fa.CIRCLE_PLUS .. u8(' Добавление нового подпункта'), _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize) then
								...
								if imgui.Button(fa.FLOPPY_DISK .. u8' Сохранить', imgui.ImVec2(imgui.GetMiddleButtonX(2), 0)) then
									...
									if lvl ~= '' and tonumber(lvl) >= 1 and tonumber(lvl) <= 6 and text ~= '' and reason ~= '' then
				...
				imgui.EndChild()
				imgui.SameLine()
				imgui.BeginChild('##smartpdd', imgui.ImVec2(292 * MONET_DPI_SCALE, 360 * MONET_DPI_SCALE), true)
			...
			if imgui.BeginTabItem(fa.GEAR..u8' Настройки') then 
				...
				imgui.SameLine()
				if imgui.SmallButton('https://www.blast.hk/') then
				...
				imgui.Separator()
				imgui.CenterText(u8'Хотите финансово поддержать MTG MODS? Реквизиты на нашем Discord сервере!')
				imgui.EndChild()
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.HANDCUFFS .. " Justice Helper - " .. fa.WALKIE_TALKIE .. u8" Рация депортамента", DeportamentWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize)
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.HANDCUFFS.." Justice Helper##rank", GiveRankMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.HANDCUFFS.." Justice Helper##CommandStopWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.HANDCUFFS.." Justice Helper##CommandPauseWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.HANDCUFFS..u8" Выберите игрока##fast_menu_players", FastMenuPlayers, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize  )
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.HANDCUFFS .. u8" Justice Helper - Оповещение##info_menu", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.AlwaysAutoResize  )
		...
		if imgui.Button(fa.CIRCLE_RIGHT..u8' Сбросить настройки ',  imgui.ImVec2(imgui.GetMiddleButtonX(2), 25 * MONET_DPI_SCALE)) then
			settings = default_settings
			save_settings()
			sampAddChatMessage('[Justice Helper] {ffffff}Настройки хелпера успешно сброшены! Перезагрузка...', message_color)
			reload_script = true
			thisScript():reload()
		end
		imgui.End()
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.CIRCLE_INFO .. u8" Justice Helper - Оповещение##need_update", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.AlwaysAutoResize  )
		imgui.CenterText(u8'У вас сейчас установлена версия ' .. u8(tostring(thisScript().version)) .. u8', но в облаке найдена версия ' .. u8(updateVer) .. ".")
		...
		if player_id ~= nil and isParamSampID(player_id) then
			...
			if imgui.Button(fa.PLAY .. u8" Начать собеседование", imgui.ImVec2(-1, 25 * MONET_DPI_SCALE)) then
				lua_thread.create(function()
					...
					wait(2000)
					sampSendChat("Вы пришли к на собеседование?")
				end)
			end
...
imgui.OnFrame(
    ...
    function(player)
        imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        imgui.SetNextWindowSize(imgui.ImVec2(600 * MONET_DPI_SCALE, 413 * MONET_DPI_SCALE), imgui.Cond.FirstUseEver)
        imgui.Begin(fa.HANDCUFFS .. u8" Justice Helper - Умная выдача розыска##sum_menu", SumMenuWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
        if smart_uk ~= nil and isParamSampID(player_id) then
			...
			imgui.InputText(u8'##name_surname', input_sum, 128) 
			--u8:decode(ffi.string(input_sum))
			imgui.Separator()
			if u8:decode(ffi.string(input_sum)) == '' then
					...
					if imgui.CollapsingHeader(u8(chapter.name)) then
						if chapter.item then 
							for _, item in ipairs(chapter.item) do
								local popup_id = fa.USER_DOCTOR .. u8' Проверка##' .. item.text .. item.lvl .. item.reason
								...
								if imgui.BeginPopupModal(popup_id, nil, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
									imgui.CenterText(u8'Перепроверьте данные перед выдачей розыска!')
									imgui.Separator()
									imgui.Text(u8'Игрок: ' .. u8(sampGetPlayerNickname(player_id)) .. ' [' .. player_id .. ']')
									imgui.Text(u8'Уровень розыска: ' .. item.lvl)
									imgui.Text(u8'Причина выдачи розыска: ' .. u8(item.reason))
									imgui.Separator()
									if imgui.Button(fa.CIRCLE_XMARK .. u8' Отмена', imgui.ImVec2(200 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
										imgui.CloseCurrentPopup()
									end
									...
									if imgui.Button(fa.STAR .. u8' Выдать розыск', imgui.ImVec2(200 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
										...
										sampAddChatMessage('/su ' .. player_id .. ' ' .. item.lvl .. ' ' .. item.reason, message_color)
										imgui.CloseCurrentPopup()
									end
									imgui.EndPopup()
								end
				...
				-- sampAddChatMessage(input_sum_decoded,-1)
				for _, chapter in ipairs(smart_uk) do
