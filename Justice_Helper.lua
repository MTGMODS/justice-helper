...
local commands = {
	commands = {
        ...
        { cmd = 'doc' , description = 'Запросить документы (FBI)' ,  text = 'Здраствуйте, я {fraction_rank} ФБР&/do Cлева на груди спец-жетон ФБР.&Прошу предъявить документ, удостоверяющий вашу личность.&/n {get_nick({arg_id})}, введите /showpass {my_id}' , arg = '{arg_id}' , enable = false , waiting = '1.200'  },
        ...
        { cmd = 'su' , description = 'Выдать розыск' ,  text = '/me достаёт свой КПК и открывает базу данных преступников&/me вносит изменения в базу данных преступинков&/do Преступник занесён в базу данных преступиков.&/su {arg_id} {arg2} {arg3}&/z {arg_id}' , arg = '{arg_id} {arg2} {arg3}' , enable = true, waiting = '1.200'  },
        { cmd = 'fsu' , description = 'Запросить выдачу розыска' ,  text = '/do Рация на тактическом поясе.&/me достаёт рацию и связавается с диспетчером&/me передаёт диспетчеру запрос на внесение человека в базу данных преступников&{my_doklad_nick} на CONTROL&Прошу обьявить в розыск {arg2} степени дело N-({arg_id}). Причина: {arg3}' , arg = '{arg_id} {arg2} {arg3}' , enable = true, waiting = '1.200'  },
		...
		{ cmd = 'inc' , description = 'Затащить в крузер' ,  text = '/me открывает заднюю дверь крузера&/todo Наклоните голову, здесь дверь*затаскивая задержанного в транспортное средство&/incar {arg_id} {arg2}&/me закрывает заднюю дверь крузера&/do Задержанный в транспортном средстве.', arg = '{arg_id} {arg2}', enable = true, waiting = '1.200'},
		{ cmd = 'ej' , description = 'Выбросить из крузера',  text = '/me открывает дверь крузера&/me помогает человеку выйти из крузера&/eject {arg_id}&/me закрывает дверь крузера', arg = '{arg_id}', enable = true, waiting = '1.200'},	
		{ cmd = 'pl' , description = 'Выбросить игрока из его т/с',  text = '/me резким ударом дубинки разбивает стело т/с задержанного&/pull {arg_id}&/me выбрасывает задержанного из его т/с и ударом дубинки оглушает его', arg = '{arg_id}', enable = true, waiting = '1.200'},	
	...
	commands_manage = {
		...
		{ cmd = 'fmutes' , description = 'Выдать мут сотруднику (10 min)' , text = '/fmutes {arg_id} {expel_reason}&/r Сотрудник {get_ru_nick({arg_id})} лишился права использовать рацию на 10 минут!' , arg = '{arg_id}', enable = true, waiting = '1.200'   },
...
function main()
	...
	if tostring(settings.general.version) ~= tostring(thisScript().version) then 
		sampAddChatMessage('[Justice Helper] {ffffff}Обнаружен конфликт версий, сброс настроек хелпера...', message_color)
		os.remove(path_settings)
		os.remove(path_uk)
		os.remove(path_pdd)
		reload_script = true
